...
interface FormularioClasseVIIProps {
  ...
  onChange: (params: ParametrosClasseVII, valorTotal: number, valorCombustivel?: number) => void;
}

export function FormularioClasseVII({ value, onChange }: FormularioClasseVIIProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {params.tipoOperacao === 'AQUISICAO' ? (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Valor Unitário de Aquisição (R$) <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              min="0"
          ...
          <>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Valor de Referência Unitário (R$) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                min="0"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Custo de Manutenção por Equipamento (R$) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                min="0"
                ...
                onChange={(e) => handleChange('custoManutencao', parseFloat(e.target.value) || 0)}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-600 focus:border-transparent"
                placeholder="0.00"
              />
              <p className="text-xs text-gray-500 mt-1">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
        <p className="text-sm text-blue-800">
          <strong>Fórmula:</strong> {params.tipoOperacao === 'AQUISICAO'
